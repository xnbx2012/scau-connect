...
async def _run_login(config: Config, start_proxy: bool) -> int:
    ...
    try:
        ...
        if http_proxy:
            ...
            ca_path = Path(".proxy-ca/local-ca.crt.pem").resolve()
            typer.echo(f"CA 证书：{ca_path}")
            typer.echo("提示：如浏览器提示您的连接不是私密连接，请将此 CA 证书安装到系统中")
            typer.echo("       Windows: 双击证书 -> 安装证书 -> 受信任的根证书颁发机构")
        if socks_proxy:
...
def cainfo() -> None:
    ...
    ca_path = Path(".proxy-ca/local-ca.crt.pem").resolve()
    if ca_path.exists():
        typer.echo(f"CA 证书路径：{ca_path}")
        typer.echo("")
        typer.echo("=== 安装说明 ===")
        typer.echo("浏览器报证书错误时，需将此 CA 证书安装到系统信任存储。")
        typer.echo("")
        typer.echo("Windows：")
        typer.echo("  1. 双击 .proxy-ca/local-ca.crt.pem")
        typer.echo('  2. 选择"安装证书"')
        typer.echo('  3. 选择"本地计算机" -> "将所有证书放入下列存储"')
        typer.echo('  4. 浏览 -> "受信任的根证书颁发机构"')
        typer.echo("")
        typer.echo("macOS：")
        typer.echo("  1. 打开钥匙串访问")
        typer.echo("  2. 导入 local-ca.crt.pem 到系统钥匙串")
        typer.echo("  3. 双击证书，展开信任，选择始终信任")
        typer.echo("")
        typer.echo("Linux：")
        typer.echo("  sudo cp .proxy-ca/local-ca.crt.pem /usr/local/share/ca-certificates/scau-connect.crt")
        typer.echo("  sudo update-ca-certificates")
        typer.echo("")
        typer.echo("curl：")
        typer.echo('  export CURL_CA_BUNDLE="$(pwd)/.proxy-ca/local-ca.crt.pem"')
        typer.echo("  curl --proxy http://<host>:1081 https://example.com")
    else:
        typer.echo("CA 证书尚未生成。请先运行 scau-connect login 启动代理。")
        raise typer.Exit(code=1)
